...
void sayData(row* a, int count) { //수식을 입력받는 함수
	int i;
	printf("수식을 입력하세요.");
	for (i = 0; i < count; i++) {
		scanf_s("%d", &a[i].coef);
...
void setData(row* a, row* a1) { //입력받은 수식을 방법 1로 바꿔주는 함수
	int i = 0;
	int x;
	int j = a[0].degree;
...
void multi_poly(row* c, row* d, row* result) //곱셈 함수
...
void add_poly(row* c, row* d, row* result) //덧셈 함수
{
