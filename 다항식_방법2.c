...
void setData(row* a, int count) { //수식을 입력받고 구조체에 초기화하는 함수
	int i;
	printf("수식을 입력하세요.");
	for (i = 0; i < count; i++) {
		scanf_s("%d", &a[i].coef);
...
void multi_poly(row* c, int count1, row*d,int count2, row* result) //곱셈 함수
...
void add_poly(row* c,int count1, row* d, int count2, row* result) //덧셈 함수
{
	int Cpos = 0;
	int Dpos = 0;
...
void getResult(row* a, int num) //계수나 차수가 0인 항을 제외하고 출력해주는 결과값 출력 함수
{
	for (int i = 0; i <= num; i++)
	{
